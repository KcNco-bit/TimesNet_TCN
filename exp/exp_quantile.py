...
class Exp_Quantile(Exp_Basic):
    ...
    def _get_data(self, flag):
        ...
        return data_set, data_loader # Class and Loader
    ...
    def train(self, setting):
        ...
        train_steps = len(train_loader) # num of samples devieded by batch size and rounded up, eg: 1000/32 = 32
        ...
        for epoch in range(self.args.train_epochs): # How much times to perform a complete pass through the training set
            # iterate through every batch in each epoch

            iter_count = 0 # iter_count will be equal to train_steps at last
            train_loss = []

            self.model.train()
            epoch_time = time.time()
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                iter_count += 1 # increase by 1 for each batch processed
            ...
            early_stopping(vali_loss, self.model, path) # save weight and bias
        ...
        with open('./loss/loss_history.json', 'w') as f:
